...
export const orderitemsService = {
    ...
    getorderitemById: async (orderItemId) => {
        try {
            const orderItem = await OrderItemModel.findById(orderItemId).populate("orderId");
            if (!orderItem) {
                throw new Error("Order item not found");
            }
            return orderItem;
    ...
    updateorderitem: async (orderItemId, orderItemData) => {
        try {
            const orderItem = await OrderItemModel.findByIdAndUpdate(orderItemId, orderItemData, {
                new: true,
                runValidators: true,
            });
            if (!orderItem) {
